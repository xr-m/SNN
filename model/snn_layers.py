...
class dual_exp_iir_cell(torch.nn.Module):
    ...
    def forward(self, current_spike, prev_states):
        ...
        current_spike = current_spike.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
...
class dual_exp_iir_layer(torch.nn.Module):
    ...
    def forward(self, input_spikes, states):
        ...
        # unbind along last dimension
        inputs = list(input_spikes.unbind(dim=-1)) # unbind()移除指定维(最后一维)后，返回一个元组，包含了沿着指定维切片后的各个切片。
        spikes = []
        length = len(inputs)
...
class dual_exp_iir_2d_layer(torch.nn.Module):
    def __init__(self, input_shape, step_num, batch_size, tau_m, tau_s, train_coefficients):
        '''
        :param input_shape: tuple (width hight) or (width, hight, depth) this should be the same as input shape,
        always on to one connection
    ...
    def forward(self, input_spikes, states):
        ...
        spike, states = self.dual_exp_iir_cell(inputs, states)
...
class neuron_cell(torch.nn.Module):
    def __init__(self, input_size, neuron_number, step_num, batch_size, tau_m,
        ...
        if self.input_type != 'axon':
            ...
            self.weight = torch.nn.Parameter(weight)                # Parameter(weight) ： weight是可训练参数
        else:
            self.weight = torch.nn.Linear(input_size, neuron_number, bias=train_bias)
        self.tau_m = torch.full((neuron_number,), tau_m, dtype=float)
...
class neuron_2d_cell(torch.nn.Module):
    def __init__(self, input_size, neuron_number, step_num, batch_size, tau_m,
                train_bias, membrane_filter, input_type='axon', reset_v=1.0):
        '''
        :param input_size: int
        ...
        if self.input_type != 'axon':
            ...
            self.weight = torch.nn.Conv2d(in_channels=input_size, out_channels=neuron_number, kernel_size=2, padding=0)
        self.tau_m = torch.full((neuron_number,), tau_m, dtype=float)
    ...
    def forward(self, current_input, prev_states):
        ...
        if self.input_type != 'axon':
            ...
            current_input = current_input.permute(0, 3, 2, 1)
            weighted_psp = self.weight(current_input)
            weighted_psp = weighted_psp.permute(0, 3, 2, 1)
...
class neuron_2d_layer(torch.nn.Module):
    def __init__(self, input_size, neuron_number, step_num, batch_size, tau_m,
    train_bias, membrane_filter, input_type='axon', reset_v=1.0):
        '''
        :param input_size:
        ...
        self.neuron_2d_cell = neuron_2d_cell(input_size, neuron_number, step_num, batch_size, tau_m,
                                        train_bias, membrane_filter, input_type, reset_v)
    ...
    def forward(self, input_spikes, states):
        ...
        inputs = input_spikes
        spikes = []
        spike, states = self.neuron_2d_cell(inputs, states)

        # for i in range(len(inputs)):            ## 对每个channel？
        #     spike, states = self.neuron_2d_cell(inputs[i], states)
        #     spikes += [spike]
        # return torch.stack(spikes, dim=-1), states
    ...
    def create_init_states(self):
        ...
        device = self.neuron_2d_cell.reset_decay.device
        init_v = torch.zeros(self.neuron_number).to(device)
        init_reset_v = torch.zeros(self.neuron_number).to(device)
